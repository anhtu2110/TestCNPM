...
document.getElementById('change_password').addEventListener('click', (e) => {
    ...
    fetch('http://localhost:8081/MyHouseDecor/Controller/ResetPasswordController.php', {
            method: 'POST',
            body: formData
        })
...
document.getElementById('update_password').addEventListener('click', (e) => {
    ...
    xhr.open('POST', 'http://localhost:8081/MyHouseDecor/Controller/Check_CodePasswordController.php', true);
    xhr.onload = function() {
        if (xhr.status === 200) {
            try {
