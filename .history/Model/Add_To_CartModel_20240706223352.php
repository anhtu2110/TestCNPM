<?php
    ...
    class Cart_Model{
        ...
        public function check_product_exist_to_add_order($UserID, $ProductID) {
            $conn = $this->db->getConnection();
            $query = "SELECT * FROM orders WHERE UserID = '$UserID' AND ProductID = '$ProductID' AND (status IS NULL OR status IN ('1', '2', '3'))";
            $result = $conn->query($query);
        ...
        public function check_product_exist_to_new_order($UserID, $ProductID) {
            $conn = $this->db->getConnection();
            $query = "SELECT * FROM orders WHERE UserID = '$UserID' AND ProductID = '$ProductID' AND status IN ('4', '5', '6', '7')";
            $result = $conn->query($query);
            if($result && $result->num_rows > 0) {
                $row = $result->fetch_assoc();
                return true;
        ...
        public function Delete_Tmp_Product($ProductID, $userID){
            $conn = $this->db->getConnection();
            $query = "DELETE FROM orders WHERE ProductID = $ProductID AND UserID = $userID";
            $result = $conn->query($query);
            return $result;
        }
