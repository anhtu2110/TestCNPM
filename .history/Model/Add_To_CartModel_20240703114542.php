<?php
    ...
    class Cart_Model{
        ...
        public function Update_Quantity_Product_Exist($UserID, $ProductID, $quantity) {
            $conn = $this->db->getConnection();
            // Thoát các biến đầu vào để tránh SQL Injection
            $UserID = mysqli_real_escape_string($conn, $UserID);
            $ProductID = mysqli_real_escape_string($conn, $ProductID);
            ...
            // Tạo câu lệnh SQL
            $query = "UPDATE orders SET Quantity = $quantity WHERE UserID = '$UserID' AND ProductID = '$ProductID'";
            $result = $conn->query($query);
        }
