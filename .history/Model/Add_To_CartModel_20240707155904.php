<?php
    ...
    class Cart_Model{
        ...
        public function Get_Total_Price_Product_Status_Not_Null($UserID) {
            $conn = $this->db->getConnection();
            // Thoát các biến đầu vào để tránh SQL Injection
            $UserID = mysqli_real_escape_string($conn, $UserID);

            // Tạo câu lệnh SQL
            $query = "SELECT SUM(Total_Price) AS Total_Price FROM orders WHERE UserID = '$UserID' AND status IN ('1', '2', '3', '4', '5', '6')";
            $result = $conn->query($query);
