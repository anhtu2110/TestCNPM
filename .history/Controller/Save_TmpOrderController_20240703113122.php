...
session_start();
    ...
    if(isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'POST'){   
        ...
        $Price = $_POST['Price'];
        $Title = $_POST['Title'];
        if($controller->get_quantity_product_exist_Controller($UserID, $IDSpham) > 0) {
            $quantity = $controller->get_quantity_product_exist_Controller($UserID, $IDSpham) + 1;
        } else {
            $quantity = 1;
        }
        $total_price = $quantity * $Price;
        $result = $controller->Save_Tmp_OrderController($Title, $IDSpham, $UserID, $username, $Price, $quantity, $total_price);
        if($result){
