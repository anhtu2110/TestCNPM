...
document.getElementById('update_information').addEventListener('click', (e) => {
    ...
    xhr.open('POST', 'http://192.168.126.128:80/MyHouseDecor/Controller/Update_ProfileController.php', true);
    xhr.onload = function() {
        if (xhr.status == 200) {
...
document.getElementById('update_file').addEventListener('click', (e) => {
    ...
    if (document.getElementById('file').files.length == 0) {
        ...
        let xhr = new XMLHttpRequest();
        xhr.open('POST', 'http://192.168.126.128:80/MyHouseDecor/Controller/Profile_Controller.php', true);
        xhr.onload = function() {
            if (xhr.status == 200) {
