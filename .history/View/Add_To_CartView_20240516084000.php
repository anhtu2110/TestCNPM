<?php
    require_once './Controller/Add_To_CartController.php';
    ...
    <div class="row">
        ...
        <div class="col-md-9">
            ...
            <div class="giatienitem d-flex">
                <p>Giá tiền :
                <p class="giatien"><?php echo $item['Price'];?> VND</p>
                </p>
            </div>
            <div class="sll d-flex">
    <p>Số lượng :</p>
    <button class="btn btn-outline-primary decrement">-</button>
    <input type="text" value="<?php echo isset($item['Quantity']) ? $item['Quantity'] : 1;?>"min='1' class="quantity">
    <button class="btn btn-outline-primary increment">+</button>
</div>

        </div>
        <div class="col-md-1 d-flex align-items-center justify-content-center">
            <input type="hidden"id='check_status' value="<?php echo isset($item['status']) ? $item['status'] : '';?>">
            <button class="btn btn-danger tam-an" data-delet-product = <?php echo $item['ProductID'];?>>Xóa Sản Phẩm</button>
        </div>
    </div>
</div>
<br>
...
<script> 
    document.querySelectorAll('.tam-an').forEach(btn =>{
        btn.addEventListener('click',(e)=>{
            e.preventDefault();
            let ProductID = btn.getAttribute('data-delet-product');
            ...
            xhr.open('POST','./Controller/Delete_TmpOrder.php',true);
            xhr.onload = ()=>{
    ...
    document.getElementById('update_cart').addEventListener('click',(e)=>{
        ...
        let xhr = new XMLHttpRequest();
        xhr.open('POST','./Controller/Add_To_CartController.php',true);
